...
type Signer interface {
	...
	RecoverSender(tx *response.Transaction) (response.Address, error)

	// SignTx signs a transaction
	SignTx(tx *response.Transaction, key response.Key) (*response.Transaction, error)
}
...
func (e *EIP1155Signer) RecoverSender(tx *response.Transaction) (response.Address, error) {
...
func (e *EIP1155Signer) SignTx(tx *response.Transaction, key response.Key) (*response.Transaction, error) {
...
func signHash(tx *response.Transaction, chainID uint64) []byte {
	...
	if tx.Type == response.TransactionDynamicFee {
		// dynamic fee uses
	...
	if tx.Type == response.TransactionAccessList {
		dst = append([]byte{0x1}, dst...)
	} else if tx.Type == response.TransactionDynamicFee {
		dst = append([]byte{0x2}, dst...)
	}

	hash := response.Keccak256(dst)
